...
class WSA(tf.keras.layers.Layer):
    ...
    def __init__(self, hidden_size, window_size, num_heads=6, drop_prob=0.03,
        ...
        super(WSA, self).__init__(**kwargs)
...
class BSTB(tf.keras.layers.Layer):
    def __init__(self, hidden_size, num_patches, num_heads, window_size=7,
        ...
        self.layer_normalization_1 = tf.keras.layers.LayerNormalization(epsilon=1e-5)
        self.layer_normalization_2 = tf.keras.layers.LayerNormalization(epsilon=1e-5)
        self.attn = WSA(
...
def reduce_dim(num_patch_x, num_patch_y, feature_num):
    def apply(x):
        x = tf.reshape(tensor=x, shape=(
            tf.shape(x)[0], num_patch_x, num_patch_y, tf.shape(x)[-1]))
        x = layers.Conv2D(filters=feature_num, kernel_size=1, strides=1, padding='same')(x)
...
class PublicDenseSTSR:
    """ Image Restoration Using Swin Transformer
    https://homes.esat.kuleuven.be/~konijn/publications/2021/Liang5.pdf
    """

    def __init__(self, image_shape, up_factor=1, patch_size=2, num_heads=6,
                 window_size=8, num_blocks=6, num_stages=3, feature_num=180):
    ...
    def __call__(self):
        ...
        for i_block in range(self.num_blocks):
            ...
            x = tf.concat([block, x], axis=-1)
            x = reduce_dim(num_patch_x, num_patch_y, feature_num)(x)

        x = tf.reshape(tensor=x, shape=(
            tf.shape(x)[0], num_patch_x, num_patch_y, tf.shape(x)[-1]))

        x = layers.Conv2D(filters=feature_num, kernel_size=3, padding='same')(x)
        ...
        x = Upsample(up_scale=2, feature_num=feature_num)(x)
        x = Upsample(up_scale=4, feature_num=feature_num)(x)
...
class Upsample(tf.keras.layers.Layer):
    def __init__(self, up_scale=2, feature_num=None):
        """
        Post-upsample using PixelShuffle.

        :param up_scale: the scale of the upsample
        :param feature_num: number of 'dim' (hidden layer)
        """
        super(Upsample, self).__init__()
        self.up_scale = up_scale
        self.feature_num = feature_num
        self.conv = layers.Conv2D(
            filters=up_scale * up_scale * feature_num,
            kernel_size=3,
            strides=1,
            padding='same'
        )
    ...
    def call(self, x, *args, **kwargs):
        x = self.conv(x)
        x = tf.nn.depth_to_space(x, self.up_scale, 'NHWC')
...
class DDSTB:
    def __init__(self, feature_num, num_stages,
                 num_patch_y, num_patch_x, num_heads,
                 window_size, shift_size, drop_prob, name=None):
        """
        Dense in Dense Swin Transformer Block
        Dense connections between BSTBs.

        :param feature_num: number of 'dim' (hidden layers)
        :param num_stages: the number of "win + shifted_win" stages
        :param num_patch_y: number of patches along y-axis
        :param num_patch_x: number of patches along x-axis
        :param num_heads: multi_head self-attention
        :param window_size: shifted window size
        :param drop_prob: drop rate
        :param name: the index of the block
        """
    ...
    def call(self, x, *args, **kwargs):
        ...
        for i_stage in range(self.num_stages):
            stage1 = BSTB(
                ...
                num_patches=(self.num_patch_y, self.num_patch_x),
                num_heads=self.num_heads,
                window_size=self.window_size,
                shift_size=0,
                drop_rate=self.drop_prob,
                name='block_' + self.name + '_stage_' + str(i_stage) + '_attn',
            )(x)

            stage1 = tf.concat([stage1, x], -1)
            stage1 = reduce_dim(self.num_patch_x, self.num_patch_y, self.feature_num)(stage1)
            ...
            stage2 = BSTB(
                ...
                shift_size=self.shift_size,
                drop_rate=self.drop_prob,
                name='block_' + self.name + '_stage_' + str(i_stage) + '_sw-attn',
            )(stage1)
            x = tf.concat([stage1, stage2], -1)
            x = reduce_dim(self.num_patch_x, self.num_patch_y, self.feature_num)(x)

        x = tf.reshape(tensor=x, shape=(
            tf.shape(x)[0], self.num_patch_x, self.num_patch_y, tf.shape(x)[-1]))

        x = layers.Conv2D(filters=self.feature_num, kernel_size=3, padding='same')(x)
        # x = layers.LeakyReLU(alpha=0.2)(x)
        x = tf.nn.gelu(x)
    ...
    def get_config(self):
        config = super(DDSTB, self).get_config()
        config.update({
            'feature_num': self.feature_num,
            'num_stages': self.num_stages,
            'num_patch_y': self.num_patch_y,
